...
const Post = ({ publication, post, morePosts }: PostProps) => {
	...
	// Build canonical from publication.url + post.slug to avoid hashnode subdomain leaking into canonical.
	// post.url from Hashnode API may be a hashnode subdomain (e.g. eplus.hashnode.dev) even when
	// the publication has a custom domain — using publication.url is always correct.
	const publicationBaseUrl = (replaceLegacyPublicationUrl(publication.url) || publication.url).replace(/\/$/, '');
	const canonicalPostUrl = `${publicationBaseUrl}/${post.slug}`;
