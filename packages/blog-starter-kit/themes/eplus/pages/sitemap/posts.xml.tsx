import { generatePostsSitemap, setSitemapHeaders } from '@starter-kit/utils/seo/sitemap-utils';
	...
	try {
		const initialData = await request<SitemapQuery, SitemapQueryVariables>(
			...
			SitemapDocument,
			{
				...
				postsCount: 50, // Tăng số lượng posts per request
				staticPagesCount: 1,
			},
		);
		...
		const xml = generatePostsSitemap(posts, domain);
