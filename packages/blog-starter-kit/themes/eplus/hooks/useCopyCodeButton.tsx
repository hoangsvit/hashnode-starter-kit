...
const useCopyCodeButton = (containerRef: React.RefObject<HTMLElement>) => {
	useEffect(() => {
		...
		const DEFAULT_CLASSES =
			'copy-button absolute top-3 right-3 text-white font-medium px-3 py-1.5 rounded-md transition-opacity duration-200 z-10 flex items-center justify-center text-xs bg-slate-700 hover:bg-slate-600 dark:bg-slate-600 dark:hover:bg-slate-500 opacity-70 hover:opacity-100';
		const SUCCESS_CLASSES =
			'copy-button absolute top-3 right-3 text-white font-medium px-3 py-1.5 rounded-md z-10 flex items-center justify-center text-xs bg-green-600 opacity-100';
		...
		const addCopyButtonToCodeBlock = (codeBlock: Element) => {
			...
			if (pre.parentElement && pre.parentElement.classList.contains('code-block-wrapper')) {
				wrapper = pre.parentElement as HTMLElement;
			} else {
			...
			wrapper.addEventListener('mouseenter', () => handleMouseInteraction(copyButton, true));
			wrapper.addEventListener('mouseleave', () => handleMouseInteraction(copyButton, false));
