...
import useCopyHeadingLink from '../hooks/use-copy-heading-link';
...
function PostView(props: any) {
	...
	const textSelectionSharerEnabled =
		post.publication?.features?.textSelectionSharer?.isEnabled ||
		typeof post.publication?.features?.textSelectionSharer?.isEnabled === 'undefined';
	...
	return (
		<main className="blog-post-detail-card pb-24">
			<article>
					...
					<div className="col-span-full lg:col-span-6 lg:col-start-2">
						...
						<div className="relative z-20 mb-8 flex flex-row flex-wrap items-center justify-center px-4 md:-mt-7 md:mb-14 md:text-lg last:md:mb-10">
							<div className="mb-5 flex w-full flex-row items-center justify-center md:mb-0 md:w-auto md:justify-start">
								<div className="mr-2 h-10 w-10 overflow-hidden rounded-full bg-slate-200 md:mr-3 md:h-12 md:w-12 dark:bg-white/20">
									<ProfileImage
										user={post.author}
										width="200"
										height="200"
										hoverDisabled={props.isPublicationPost}
									/>
								</div>
								<a
							...
							<div className="relative my-8 md:my-14">
								<CustomImage
									...
									blurDataURL={getBlurHash(
										resizeImage(post.coverImage, {
											...blurImageDimensions,
											...(post.coverImage?.isPortrait === false
												? { c: 'thumb' }
												: { fill: 'blur' }),
										}),
									)}
				...
				<div className="container relative z-20 mx-auto grid grid-flow-row grid-cols-8 xl:gap-6 2xl:grid-cols-10"></div>
			</article>
		</main>
	);
}
